...
class Core {
    ...
    start = () => {
        ...
        app.use((err, req, res, next) => err ? utils_1.log.error(`Proxy: ${err.message}`) && res.status(500).send(`Proxy: ${err.message}`) : next());
        ...
        if (typeof this.redis.Sub === 'object') {
            ...
            this.redis.Sub.subscribe(this.config.redisChannel, (err, e) => {
                err ? utils_1.log.error(err.message) : utils_1.log.info(`Subscribed channels: ${e}`);
            });
