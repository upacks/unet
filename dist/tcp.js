...
class NetClient {
    ...
    restart = () => {
        ...
        try {
            utils_1.log.warn(`NetClient: Removing current connections and listeners ...`);
            this.client.removeAllListeners();
            this.client.destroy();
        }
        catch (err) {
            utils_1.log.error(`NetClient: While Removing current connections: ${err.message}`);
        }
        finally {
            (0, utils_1.Delay)(() => {
    ...
    start = () => {
        utils_1.log.success(`NetClient: Starting a new NET.SOCKET ...`);
        this.client = new net_1.default.Socket();
        this.client.authenticate = (token) => this.client.write(token);
        this.client.connect(this.config, () => {
            utils_1.log.success(`NetClient: Connection established with the server`);
        ...
        this.client.on('error', (err) => {
            utils_1.log.error(`NetClient: On.Error / ${err.message}`);
            this.restart();
        });
        this.client.on('close', () => {
            utils_1.log.warn(`NetClient: On.Close triggered!`);
            this.restart();
        });
        this.client.on('end', () => {
            utils_1.log.warn(`NetClient: On.End triggered!`);
            this.restart();
        });
    };
}
