    ...
    pull = /** PULL METHOD */ {
        ...
        get_last: async ({ model, table_name, slave_name, retain }, {}) => {
            const item = await model.findOne({
                where: this.kv.hasOwnProperty(slave_name) ?
                    { src: { [util_1.Op.not]: slave_name }, updatedAt: { [util_1.Op.gte]: this.kv[slave_name].updatedAt } } :
                    { src: { [util_1.Op.not]: slave_name }, updatedAt: { [util_1.Op.gte]: (0, utils_1.moment)().add(-90, 'days').format(utils_1.dateFormat) } },
            ...
            const last = { id: item?.id ?? '', updatedAt: item?.updatedAt ?? (0, utils_1.moment)().add(-(retain[0]), retain[1]).format(utils_1.dateFormat) };
        ...
        save_items: async ({ model, table_name, slave_name }, { pull_items }) => {
            ...
            for (const x of pull_items)
                await model.upsert({ ...x, createdAt: (0, utils_1.Now)() });
            return 'Done';
        },
    };
    push = /** PUSH METHOD */ {
