...
class rSlave {
    ...
    cb = null;
    constructor(args) {
        utils_1.log.warn(`[ReplicaSlave] Initializing`);
        this._ = {
            api: null,
            sequel: null,
            slave_name: '',
    ...
    push = /** PUSH METHOD */ {
        ...
        send_items: ({ key, table_name, slave_name }, { push_items }) => new Promise((res, rej) => {
            this._.api.cio.timeout(10 * 1000).emit('send_items', (0, common_1.zip)({ key, table_name, slave_name, items: push_items }), (err, response) => {
                try {
                    if (err)
                        rej(err.message);
                    else
        ...
        (0, utils_1.Loop)(() => free && (0, utils_1.Safe)(async () => {
            ...
            try {
                if (typeof skip[index] === 'number' && skip[index] > 0) {
                    ...
                    const arg = { key, index, model, table_name: name, master_name: 'master', slave_name: this._.slave_name, retain, size };
